...
const AppointmentModal = ({ isOpen, onClose, initialData = {} }) => {
  ...
  // Helper to filter dentists based on selected dental treatment
  const filterDoctorsByDept = (deptName) => {
    ...
    const matched = chiefDoctorsList.filter((doc) => {
      ...
      return docDept.includes(lowerDept) || lowerDept.includes(docDept) ||
        (lowerDept.includes('implant') && (docDept.includes('implant') || docDept.includes('prostho') || docDept.includes('surgery'))) ||
        (lowerDept.includes('invisalign') && (docDept.includes('ortho') || docDept.includes('aligner'))) ||
        (lowerDept.includes('align') && docDept.includes('ortho')) ||
        (lowerDept.includes('root canal') && (docDept.includes('endo') || docDept.includes('conservative'))) ||
        (lowerDept.includes('veneer') && (docDept.includes('prostho') || docDept.includes('cosmetic') || docDept.includes('aesthetic'))) ||
        (lowerDept.includes('white') && (docDept.includes('periodont') || docDept.includes('aesthetic'))) ||
        (lowerDept.includes('pediatric') && docDept.includes('pediatric')) ||
        (lowerDept.includes('gum') && (docDept.includes('periodont') || docDept.includes('laser'))) ||
        (lowerDept.includes('wisdom') && (docDept.includes('surgery') || docDept.includes('maxillo'))) ||
        (lowerDept.includes('crown') && (docDept.includes('prostho') || docDept.includes('cadcam')));
    });
  ...
  const [formData, setFormData] = useState({
    ...
    age: '',
    gender: 'Female',
    department: initialData?.department || (servicesList[0]?.title || 'Dental Implants & All-on-4 / All-on-6 Restoration'),
    doctorName: initialData?.doctorName || (chiefDoctorsList[0]?.name || 'Dr. Arthur Vance, MDS, FICOI (USA)'),
    date: initialData?.date || new Date(Date.now() + 86400000).toISOString().split('T')[0],
    timeSlot: '10:00 AM',
    sessionType: 'In-Clinic Dental Specialist Consultation & 3D Exam',
    consultationFee: 500,
    paymentMethod: 'Pay at Dental Counter / UPI',
    symptomsNotes: '',
    branchName: 'DentaCare Central Hospital & Surgical Center'
  });
  ...
  const timeSlots = [
    ...
    '05:00 PM', '05:30 PM', '06:00 PM', '06:30 PM', '07:30 PM'
  ];

  // Sync when initialData changes
  useEffect(() => {
    if (isOpen) {
      const dept = initialData?.department || servicesList[0]?.title || 'Dental Implants & All-on-4 / All-on-6 Restoration';
  ...
  // Generate Unique Sequential Token ID (DENT001, DENT002, ...)
  const generateAppointmentToken = () => {
    const existingCount = appointments.length;
    const lastStored = parseInt(localStorage.getItem('dentacare_last_apt_num') || '0', 10);
    const nextNum = Math.max(existingCount, lastStored) + 1;
    localStorage.setItem('dentacare_last_apt_num', nextNum.toString());
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.patientName.trim() || !formData.mobileNumber.trim()) {
      alert('Please provide your full name and mobile phone number.');
      return;
    }
    ...
    const newAppointment = {
      ...
      symptoms: formData.symptomsNotes || 'Dental Checkup & Smile Consultation',
  ...
  const handleDownload = () => {
    ...
    const text = `
=====================================================
    DENTACARE SPECIALISTS & ADVANCED IMPLANT CENTER
        OFFICIAL DENTAL CONSULTATION CONFIRMATION SLIP
=====================================================
...
-----------------------------------------------------
DENTAL PROCEDURE   : ${successReceipt.department}
...
STATUS             : CONFIRMED (PRIORITY OPERATORY QUEUE)
-----------------------------------------------------
...
Complimentary RVG Digital Bitewing X-Ray included.
    `;
    const element = document.createElement("a");
    const file = new Blob([text], { type: 'text/plain' });
    element.href = URL.createObjectURL(file);
    element.download = `DentaCare_Token_${successReceipt.token}.txt`;
  ...
  const handleWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      `Hello DentaCare, I have booked a Dental Appointment. Token #${successReceipt.token} for ${successReceipt.patientName} with ${successReceipt.doctorName} on ${successReceipt.date} at ${successReceipt.time}.`
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-cyan-100 overflow-hidden my-8">
        
        {/* MODAL HEADER */}
        <div className="bg-gradient-to-r from-slate-900 via-cyan-950 to-slate-950 text-white p-6 relative">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center space-x-2 text-cyan-400 text-xs font-bold uppercase tracking-wider mb-1">
            ...
            <span>Institute of Advanced Dentistry & Smile Aesthetics</span>
          </div>
          
          <h2 className="text-xl sm:text-2xl font-black font-heading text-white">
            {isSuccess ? 'Dental Consultation Token Confirmed' : 'Book Specialist Dentist / 3D Consultation'}
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 mt-1">
            {isSuccess 
              ? 'Your appointment is confirmed in our clinical operatory queue.' 
              : 'Consult board-certified implantologists, orthodontists, and root canal specialists.'}
          </p>
        </div>
        ...
        <div className="p-6 sm:p-8 max-h-[75vh] overflow-y-auto">
          ...
          {isSuccess && successReceipt ? (
            ...
            <div className="space-y-6 animate-in zoom-in-95 duration-200">
              ...
              <div className="bg-cyan-50 border border-cyan-200 rounded-2xl p-5 text-center space-y-2">
                <div className="w-12 h-12 bg-cyan-600 text-white rounded-full flex items-center justify-center mx-auto shadow-md">
                  <CheckCircle2 className="w-7 h-7" />
                </div>
                <h3 className="text-lg font-extrabold text-slate-950 font-heading">
                  Dental Consultation Token #{successReceipt.token} Generated!
                </h3>
                <p className="text-xs text-slate-700">
                  Confirmed for <span className="font-bold text-cyan-700">{successReceipt.patientName}</span> with <span className="font-bold text-slate-900">{successReceipt.doctorName}</span>.
                </p>
              </div>

              {/* Digital Slip Card */}
              <div className="border border-cyan-100 rounded-2xl p-5 bg-cyan-50/30 space-y-4 text-xs">
                <div className="flex items-center justify-between border-b border-cyan-100 pb-3">
                  <div>
                    ...
                    <span className="text-2xl font-black text-cyan-600 font-mono tracking-tight">{successReceipt.token}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-slate-700">
                  ...
                  <div>
                    <span className="text-slate-500 block text-[11px]">Treatment / Speciality</span>
                    <span className="font-bold text-slate-900">{successReceipt.department}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block text-[11px]">Specialist Dentist</span>
                  ...
                  <div>
                    ...
                    <span className="font-bold text-cyan-700">{successReceipt.date} at {successReceipt.time}</span>
                  </div>
                  <div>
                ...
                <div className="pt-2 border-t border-cyan-100 flex items-center justify-between text-[11px] text-slate-500">
                  ...
                  <span>☎ Emergency Helpline: 1800-419-6784</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <button
                  onClick={handlePrint}
                  className="flex items-center justify-center space-x-2 py-3 px-4 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-xl text-xs shadow-sm transition-all cursor-pointer"
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Row 1: Treatment & Dentist */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1.5">
                    Treatment / Dental Specialty
                  </label>
                  <select
                    value={formData.department}
                    onChange={(e) => handleDepartmentChange(e.target.value)}
                    className="w-full text-xs font-medium px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white transition-colors"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white transition-colors"
                  />
                </div>
              ...
              {/* Row 6: Chief Dental Complaint Notes */}
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1.5">
                  Chief Dental Concern / Symptoms (Optional)
                </label>
                <textarea
                  rows="2"
                  placeholder="e.g. Severe throbbing toothache in lower molar, missing tooth on right side, interested in clear aligners, bleeding gums..."
                  value={formData.symptomsNotes}
                  onChange={(e) => setFormData({ ...formData, symptomsNotes: e.target.value })}
                  className="w-full text-xs px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white transition-colors"
                ></textarea>
              </div>

              {/* Consultation Fee & Security Banner */}
              <div className="bg-cyan-50/60 border border-cyan-100 rounded-xl p-3.5 flex items-center justify-between text-xs">
                <div>
                  <span className="text-slate-500 block text-[11px]">Consultation Fee</span>
                  <span className="font-extrabold text-slate-900 text-sm">₹ {formData.consultationFee} (Pay at Dental Counter / UPI)</span>
                </div>
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  className="w-full py-3.5 px-4 bg-gradient-to-r from-cyan-600 via-sky-600 to-cyan-700 hover:from-cyan-700 hover:to-sky-700 text-white font-bold rounded-xl text-sm shadow-md shadow-cyan-200 hover:shadow-lg transition-all active:scale-[0.99] cursor-pointer"
                >
                  Confirm Dental Consultation Token
                </button>
              </div>
