    ...
    <footer className="bg-slate-950 text-slate-300 pt-16 pb-8 border-t border-slate-800 text-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 pb-12 border-b border-slate-800">
          ...
          <div className="lg:col-span-2 space-y-4">
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="w-10 h-10 bg-gradient-to-br from-cyan-500 via-sky-600 to-cyan-700 rounded-xl flex items-center justify-center text-white shadow-md shadow-cyan-950">
              ...
              <div>
                ...
                <span className="text-[10px] font-bold text-cyan-400 tracking-wider uppercase block -mt-0.5">
                  {hospitalInfo.tagline || 'Institute of Advanced Dentistry & Implantology'}
                </span>
              </div>
            </Link>
            ...
            <div className="pt-2 flex flex-wrap items-center gap-2">
              <div className="flex items-center space-x-1.5 text-[11px] text-slate-300 font-medium bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800">
                ...
                <span>ISO 9001 & Class B Sterile Hospital</span>
              </div>
              <div className="flex items-center space-x-1.5 text-[11px] text-slate-300 font-medium bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800">
                <Award className="w-4 h-4 text-amber-400" />
                <span>Board-Certified MDS Dental Specialists</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2">
              {[
                { to: '/', label: 'Home Page' },
                { to: '/branches', label: 'Dental Clinics & Hubs' },
                { to: '/technologies', label: '3D CBCT & Laser Tech' },
                { to: '/health-center/master-health-checkup', label: 'Smile Checkup Packages' },
                { to: '/blog', label: 'Dental Care Journal' },
                { to: '/about', label: 'About DentaCare Specialists' },
                { to: '/insurances', label: '0% EMI & Dental Insurance' },
                { to: '/contact', label: 'Emergency & Appointments' },
                { to: '/admin-panel-login', label: 'Doctor & Staff Admin Login' },
              ].map((link) => (
                <li key={link.to}>
                  <Link to={link.to} className="text-slate-400 hover:text-cyan-400 transition-colors flex items-center space-x-1">
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2">
              {servicesList.slice(0, 6).map((service) => (
                <li key={service.id}>
                  <Link to={`/services/${service.slug}`} className="text-slate-400 hover:text-cyan-400 transition-colors flex items-center space-x-1">
                    <ChevronRight className="w-3 h-3 text-slate-500" />
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              <a href={`tel:${hospitalInfo.emergencyNumber || '1800-419-6784'}`} className="flex items-center space-x-2 text-cyan-400 hover:text-cyan-300 font-bold bg-cyan-950/40 p-2.5 rounded-xl border border-cyan-900/50">
                <PhoneCall className="w-4 h-4 shrink-0 animate-pulse" />
                <span>24/7 Helpline: {hospitalInfo.emergencyNumber || '1800-419-6784'}</span>
              </a>
        ...
        <div className="pt-6 flex flex-col md:flex-row items-center justify-between text-slate-500 gap-y-2 text-[11px]">
          <div>
            © {new Date().getFullYear()} {hospitalInfo.fullName || `${hospitalInfo.name} Specialists Hospital & Center for Advanced Implantology`}. All Rights Reserved.
          </div>
          <div className="flex items-center space-x-4">
            <a href="#" className="hover:text-slate-300 transition-colors">Privacy Policy</a>
            <a href="#" className="hover:text-slate-300 transition-colors">Class B Sterilization Protocol</a>
            <a href="#" className="hover:text-slate-300 transition-colors">Implant Lifetime Warranty Seal</a>
            <Link to="/admin-panel-login" className="hover:text-cyan-400 transition-colors">Doctor Admin Login</Link>
          </div>
        </div>
