...
import { MapPin, Phone, Mail, Clock, CheckCircle, Calendar, Sparkles, ShieldCheck, Smile } from 'lucide-react';
...
const BranchPage = ({ onOpenAppointment }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16 space-y-10">
      <PageHero
        title={branch.name}
        subtitle={`${branch.city} • Advanced Dental Implantology, Orthodontics & Surgical Clinic`}
        breadcrumb={[{ label: 'Clinics & Hubs', path: '/branches' }, { label: branch.name }]}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-4">
            ...
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Equipped with Class B sterile operatory suites, 3D CBCT digital diagnostics, Carl Zeiss operating microscopes, and board-certified dental surgeons.
            </p>
            ...
            <div className="pt-4 flex items-center space-x-3">
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-700 hover:to-sky-700 text-white font-bold text-xs rounded-xl shadow-sm shadow-cyan-200 flex items-center space-x-2 cursor-pointer transition-all"
              >
                <Calendar className="w-4 h-4" />
                <span>Book Appointment at this Center</span>
              </button>
            </div>
          </div>
        ...
        {(branch.features || branch.facilities) && (
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 space-y-4">
            <h3 className="text-base font-bold text-slate-950 font-heading">Dental Operatory & Clinical Infrastructure</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {(branch.features || branch.facilities).map((service, idx) => (
                <div key={idx} className="p-3.5 bg-cyan-50/50 rounded-2xl border border-cyan-100 flex items-center space-x-3">
