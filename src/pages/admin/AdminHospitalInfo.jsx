...
import { Building2, Save, RotateCcw, Sparkles, Smile } from 'lucide-react';
    ...
    <div className="space-y-5">
      <div className="border-b border-slate-200 pb-3">
        <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
          ...
          <span>Hospital Profile & Emergency Settings</span>
        </h1>
        <p className="text-xs text-slate-500 mt-0.5">Edit global dental branding, emergency toothache helpline details, and main hospital campus address.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          {[
            { key: 'name', label: 'Hospital Brand Name', placeholder: 'DENTACARE' },
            { key: 'tagline', label: 'Tagline', placeholder: 'Institute of Advanced Dentistry, Implantology & Smile Aesthetics' },
            { key: 'fullName', label: 'Full Official Entity Name', placeholder: 'DentaCare Specialists Hospital & Center for Advanced Implantology' },
            { key: 'email', label: 'Primary Contact Email', placeholder: 'care@dentacarespecialists.org' },
            { key: 'phone', label: 'Appointment Desk Phone', placeholder: '+91 98401 23456' },
            { key: 'emergencyNumber', label: '24/7 Toothache & Emergency Hotline', placeholder: '1800-419-6784' },
        ...
        <div className="text-xs">
          <label className="block font-bold text-slate-700 mb-1">Main Hospital Campus Address</label>
          <textarea
            rows={2}
            value={form.address || ''}
            onChange={(e) => setForm({ ...form, address: e.target.value })}
            placeholder="Main dental hospital street address, landmark, city, and pincode..."
