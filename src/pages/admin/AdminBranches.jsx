...
import { MapPin, Plus, Pencil, Trash2, X, Save, Search, Sparkles, Smile } from 'lucide-react';
...
const AdminBranches = () => {
  ...
  const openAdd = () => { 
    ...
    setForm({ 
      ...
      timing: 'Mon - Sat: 08:00 AM - 09:00 PM (Sun: 09:00 AM - 02:00 PM)', 
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Dental Hospitals & Specialized Clinic Centers</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage dental clinic locations, 3D imaging suites, and consultation hours (Total: {branches.length})</p>
        </div>
        <button onClick={openAdd} className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-700 hover:to-sky-700 text-white font-bold text-xs rounded-xl flex items-center space-x-1.5 transition-all cursor-pointer shadow-sm shadow-cyan-200">
          <Plus className="w-4 h-4" /><span>Add Dental Clinic</span>
        </button>
      </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          placeholder="Search dental clinics and cities..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filtered.map((branch) => (
          <div key={branch.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden hover:border-cyan-400 hover:shadow-md transition-all flex flex-col justify-between">
            ...
            <div className="p-6 space-y-2 flex-grow">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <span className="text-[11px] text-cyan-600 font-semibold">{branch.city}</span>
                </div>
                <div className="flex items-center space-x-1 shrink-0">
                  <button onClick={() => openEdit(branch)} className="p-1.5 text-slate-500 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg cursor-pointer transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => setDeleteConfirm(branch.id)} className="p-1.5 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg cursor-pointer transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-950 font-heading">
                {editingId ? 'Edit Dental Clinic' : 'Add New Dental Clinic'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Dental Clinic Name *</label>
                <input
                  type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="e.g. DentaCare Flagship Hospital & 3D Center" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Planmeca 3D CBCT, 4 Surgical Operatories, Carl Zeiss Micro-Endo"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
