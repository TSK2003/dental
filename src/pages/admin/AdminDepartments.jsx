...
import { Layers3, Plus, Pencil, Trash2, X, Save, Search, Sparkles, Smile } from 'lucide-react';
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Specialized Dental Treatments & 3D Surgical Modules</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage clinical dental procedures, 3D implant suites, and orthodontic protocols (Total: {services.length})</p>
        </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          placeholder="Search dental specialties and surgical procedures..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map((svc) => (
          <div key={svc.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-3 hover:border-cyan-400 hover:shadow-md transition-all flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex items-start justify-between">
                <h3 className="font-bold text-sm text-slate-950 font-heading leading-snug">{svc.title}</h3>
                <div className="flex items-center space-x-1">
                  <button onClick={() => openEdit(svc)} className="p-1.5 text-slate-500 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg cursor-pointer transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
              <span className="font-medium text-cyan-600">/services/{svc.slug}</span>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-950 font-heading">
                {editingId ? 'Edit Dental Procedure' : 'Add New Dental Procedure'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 3D Guided Dental Implants & Restoration" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. dental-implants-restoration" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Brief 1-2 sentence clinical summary" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
