...
import { FileText, Plus, Pencil, Trash2, X, Save, Search, Sparkles, Smile } from 'lucide-react';
...
const AdminBlog = () => {
  ...
  const openAdd = () => { 
    ...
    setForm({ 
      ...
      excerpt: '', 
      content: '', 
      author: 'Dr. Arthur Vance, MDS, FICOI (USA)', 
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Dental Health Journal & Clinical Articles</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage dental health guides, implant recovery tips, and oral care articles (Total: {(blogPosts || []).length})</p>
        </div>
        <button onClick={openAdd} className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-700 hover:to-sky-700 text-white font-bold text-xs rounded-xl flex items-center space-x-1.5 transition-all cursor-pointer shadow-sm shadow-cyan-200">
          <Plus className="w-4 h-4" /><span>Add Dental Article</span>
        </button>
      </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          placeholder="Search dental journal by article title..."
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 text-left border-b border-slate-200 font-bold text-slate-700 uppercase text-[10px] tracking-wider">
                ...
                <th className="px-5 py-3.5">Author Dentist</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((post) => (
                <tr key={post.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-5 py-4"><span className="px-2.5 py-0.5 bg-cyan-50 text-cyan-800 rounded-md font-bold text-[11px]">{post.category}</span></td>
                  ...
                  <td className="px-5 py-4 text-right space-x-1.5">
                    <button onClick={() => openEdit(post)} className="text-slate-500 hover:text-cyan-600 p-1.5 rounded-lg hover:bg-cyan-50 cursor-pointer transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 space-y-4 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-950 font-heading">
                {editingId ? 'Edit Dental Article' : 'Write New Dental Journal Article'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 3D Guided Dental Implants vs Traditional: Why Keyhole Surgery Matters" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Dental Implants" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                  />
                </div>
                <div>
