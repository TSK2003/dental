...
import { UserRound, Plus, Pencil, Trash2, X, Save, Search, Sparkles, Smile } from 'lucide-react';

const emptyDoctor = {
  name: '', 
  qualification: 'MDS, FICOI (USA), Oral Implantology', 
  department: 'Center for Guided Implantology & Full Mouth Rehab', 
  branchId: 'main-campus', 
  experience: '12+ Years',
  image: 'https://images.unsplash.com/photo-1622253692010-333f2da6031d?auto=format&fit=crop&w=800&q=80',
  bio: 'Specialist in 3D CBCT computer-guided dental implants, bone grafting, and immediate fixed restorations.', 
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Specialist Dental Surgeons & Orthodontists Directory</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage consultant dentists, MDS qualifications, fellowships, and OPD operatory hours (Total: {doctors.length})</p>
        </div>
        <button onClick={openAdd} className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-700 hover:to-sky-700 text-white font-bold text-xs rounded-xl flex items-center space-x-1.5 transition-all cursor-pointer shadow-sm shadow-cyan-200">
          ...
          <span>Add Dental Specialist</span>
        </button>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by doctor name or dental subspecialty..."
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 text-left border-b border-slate-200 font-bold text-slate-700 uppercase text-[10px] tracking-wider">
                <th className="px-5 py-3.5">Specialist Dentist</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredDoctors.map((doc) => (
                <tr key={doc.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-5 py-4 font-semibold text-cyan-600">{doc.department}</td>
                  ...
                  <td className="px-5 py-4 text-right space-x-1.5">
                    <button onClick={() => openEdit(doc)} className="p-1.5 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors cursor-pointer">
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-950 font-heading">
                {editingId ? 'Edit Dental Specialist Profile' : 'Add New Dental Specialist'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Dentist Name *</label>
                <input
                  type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="e.g. Dr. Arthur Vance, MDS" className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                />
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl border border-slate-200 text-center">
            <h4 className="text-sm font-bold text-slate-900">Delete Dentist Profile?</h4>
            <p className="text-xs text-slate-500">Are you sure you want to remove this dentist from the portal?</p>
