...
import { Award, Save, RotateCcw, Plus, Trash2, Sparkles, Smile } from 'lucide-react';
...
const AdminWhyChooseUs = () => {
  ...
  const addItem = () => {
    setItems([...items, { title: 'New Dental Advantage', desc: 'Description of clinical dental excellence...', icon: 'Smile' }]);
  };
  ...
  const iconOptions = ['Smile', 'ShieldCheck', 'Cpu', 'Award', 'Zap', 'Building2', 'Heart', 'Clock'];
    ...
    <div className="space-y-5">
      <div className="border-b border-slate-200 pb-3">
        <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
          ...
          <span>Why Choose Us Dental Highlights</span>
        </h1>
        <p className="text-xs text-slate-500 mt-0.5">Customize the institutional dental trust highlights and clinical advantages displayed on the landing page.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        {items.map((item, idx) => (
          <div key={idx} className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <span className="text-[11px] font-bold text-cyan-600 uppercase tracking-wider">Highlight Card #{idx + 1}</span>
              <button type="button" onClick={() => removeItem(idx)} className="p-1 text-slate-400 hover:text-cyan-600 rounded-lg cursor-pointer">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-cyan-500 focus:bg-white font-semibold"
                >
        ...
        >
          ...
          <span>Add New Dental Advantage Card</span>
        </button>
