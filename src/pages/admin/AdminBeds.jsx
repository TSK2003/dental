...
const AdminBeds = () => {
  ...
  const [admitForm, setAdmitForm] = useState({
    patientName: '',
    patientId: '',
    attendingDoctor: doctors[0]?.name || 'Dr. Arthur Vance, MDS, FICOI (USA)',
    surgicalProcedure: 'Nobel Biocare® 3D Guided Dental Implant Placement',
    notes: ''
  });
  ...
  const [newBedForm, setNewBedForm] = useState({
    bedNumber: '',
    ward: 'IMPLANT_SUITE',
    category: '3D Guided Implant Surgery Operatory',
    floor: '2nd Floor',
    wing: 'Surgical Implantology Wing',
    dailyRate: 3500,
    equipment: 'Planmeca ProMax 3D + KaVo ESTETICA E80 Dental Chair',
    notes: 'Sterile Class B surgical operatory with HEPA positive-pressure'
  });
  ...
  const handleOpenAdmit = (bed) => {
    ...
    setAdmitForm({
      patientName: '',
      patientId: `DNT-${Math.floor(90000 + Math.random() * 9999)}`,
      attendingDoctor: doctors[0]?.name || 'Dr. Arthur Vance, MDS, FICOI (USA)',
      surgicalProcedure: bed.condition || '3D Guided Implant & Scan',
      notes: ''
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-slate-200 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Dental Operatory Suites & Surgical Chair Management</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Real-time tracking of 3D Guided Implant suites, Carl Zeiss Micro-Endo bays, and Orthodontic aligner studios.
          </p>
        </div>
        <button
          onClick={() => setShowAddBedModal(true)}
          className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-700 hover:to-sky-700 text-white font-bold text-xs rounded-xl shadow-sm shadow-cyan-200 transition-all flex items-center space-x-1.5 cursor-pointer w-fit"
        >
          <Plus className="w-4 h-4" />
          <span>Add New Operatory / Chair</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-1">
          <span className="text-[11px] font-bold text-slate-400 uppercase">Total Operatories</span>
          <p className="text-2xl font-black text-slate-950 font-heading">{totalBeds}</p>
          <span className="text-[10px] text-cyan-600 font-semibold">{occupancyRate}% Chair Utilization</span>
        </div>
        <div className="bg-amber-50/60 p-5 rounded-3xl border border-amber-200 shadow-sm space-y-1">
          <span className="text-[11px] font-bold text-amber-800 uppercase">In-Procedure</span>
        ...
        <div className="bg-cyan-50/60 p-5 rounded-3xl border border-cyan-200 shadow-sm space-y-1">
          <span className="text-[11px] font-bold text-cyan-800 uppercase">Autoclave Sterilization</span>
          <p className="text-2xl font-black text-cyan-900 font-heading">{sanitizingBeds + maintenanceBeds}</p>
          <span className="text-[10px] text-cyan-600 font-medium">Class B Protocol</span>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by operatory number, patient name, UHID, or dental surgeon..."
        ...
        <select
          ...
          className="px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-semibold text-slate-700 focus:outline-none focus:border-cyan-500 shadow-2xs"
        >
          <option value="ALL">All Chair Statuses</option>
          <option value="available">Ready / Vacant</option>
          <option value="occupied">In-Procedure</option>
            ...
            <div
              ...
              className={`bg-white rounded-3xl p-6 border transition-all flex flex-col justify-between shadow-sm ${
                isOccupied
                  ...
                  : 'border-cyan-300 bg-cyan-50/20 hover:border-cyan-500'
              }`}
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                    isOccupied ? 'bg-amber-100 text-amber-800' : isAvailable ? 'bg-emerald-100 text-emerald-800' : 'bg-cyan-100 text-cyan-800'
                  }`}>
                    {bed.status}
                  </span>
                </div>
                ...
                {isOccupied ? (
                  <div className="p-3 bg-white rounded-2xl border border-amber-200 space-y-1 text-xs">
                    ...
                    <p className="text-cyan-600 font-semibold text-[11px]">{bed.condition}</p>
                    <p className="text-slate-500 text-[11px]">Dentist: {bed.attendingDoctor}</p>
                  ...
                  <div className="p-3 bg-white rounded-2xl border border-slate-100 space-y-1 text-xs text-slate-500">
                    <p className="text-[11px]">Procedure Rate: <span className="font-bold text-slate-800">₹{bed.dailyRate}</span></p>
                    <p className="text-[10px] text-slate-400">Class B Sterile Dental Zone</p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 mt-4 flex items-center justify-between gap-2">
                {isAvailable ? (
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs rounded-xl shadow-xs flex items-center justify-center space-x-1.5 transition-colors cursor-pointer"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Mark Autoclaved & Ready</span>
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-950 font-heading">Assign Patient to {admitModalBed.bedNumber}</h3>
            ...
            <form onSubmit={handleConfirmAdmission} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-cyan-500"
                />
              </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Procedure / Treatment *</label>
                <input
                  type="text"
                  required
                  value={admitForm.surgicalProcedure}
                  onChange={(e) => setAdmitForm({ ...admitForm, surgicalProcedure: e.target.value })}
                  placeholder="e.g. 3D Guided Implant & Scan"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-cyan-500"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1">Consulting Dentist</label>
                <select
              ...
              <div className="pt-3 flex space-x-2">
                ...
                <button
                  type="submit"
                  className="w-1/2 py-2.5 bg-gradient-to-r from-cyan-600 to-sky-600 text-white font-bold rounded-xl shadow-sm shadow-cyan-200 cursor-pointer"
                >
                  Confirm Chair Allocation
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            ...
            <form onSubmit={handleConfirmTransfer} className="space-y-4 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Select Destination Operatory *</label>
                <select
                  required
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                >
                  <option value="">-- Choose Available Operatory --</option>
              ...
              <div className="pt-3 flex space-x-2">
                ...
                <button
                  ...
                  className="w-1/2 py-2.5 bg-cyan-600 disabled:opacity-50 text-white font-bold rounded-xl shadow-sm cursor-pointer"
                >
                  Complete Transfer
                </button>
              </div>
      ...
      {showAddBedModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-950 font-heading">Add New Dental Operatory Suite</h3>
            ...
            <form onSubmit={handleAddBedSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Operatory Suite Number *</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. OPERATORY-101 or IMPLANT-SUITE-1"
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Operatory Category *</label>
                <select
                  value={newBedForm.ward}
                  onChange={(e) => setNewBedForm({ ...newBedForm, ward: e.target.value, category: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl"
                >
                  <option value="IMPLANT_SUITE">3D Guided Implant Surgery Operatory</option>
                  <option value="MICRO_ENDO_SUITE">Carl Zeiss Micro-Endodontics Suite</option>
                  <option value="ORTHO_BAY">Invisalign® & Orthodontic Bay</option>
                  <option value="PEDIATRIC_STUDIO">Pediatric Smile Studio</option>
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Procedure Rate (₹)</label>
                  <input
                    type="number"
