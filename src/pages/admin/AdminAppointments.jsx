...
import { CalendarCheck, Search, Eye, CheckCircle2, XCircle, Printer, Download, Trash2, Sparkles, Smile } from 'lucide-react';
      ...
      <div className="border-b border-slate-200 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center space-x-2 font-heading">
            ...
            <span>Dental Consultation & Operatory Token Queue</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Real-time tracking of booked Dental Implants, Invisalign, Microscopic Root Canal, and Oral Surgery appointments (Total: {appointments.length})</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input 
            ...
            placeholder="Search by patient name, token ID, or dental surgeon..."
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 text-left border-b border-slate-200 font-bold text-slate-700 uppercase text-[10px] tracking-wider">
                ...
                <th className="px-5 py-3.5">Consultant Dentist</th>
                <th className="px-5 py-3.5">Specialty / Procedure</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((apt, idx) => {
                const id = apt.appointmentId || apt.token || `DENT${String(idx + 1).padStart(3, '0')}`;
                ...
                return (
                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-5 py-4 font-bold text-cyan-600 font-mono">{id}</td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <div className="text-[10px] text-cyan-600 font-semibold">{apt.department}</div>
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      <span className="font-semibold text-slate-800">{apt.sessionType || 'Operatory Consultation'}</span>
                      <span className="text-[10px] text-slate-400 block">{apt.branchName || 'Central Dental Hub'}</span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="font-semibold text-slate-800">{apt.date}</div>
                      <div className="text-[10px] text-cyan-600 font-medium">{time}</div>
                    </td>
                    <td className="px-5 py-4">
